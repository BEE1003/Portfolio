            ...
            <div>
              ...
              <ol className="mt-8 space-y-8 border-l border-border pl-6">
                ...
                <li className="relative">
                  ...
                  <p className="mt-2 text-sm sm:text-base leading-relaxed text-muted-foreground">
                    負責程式設計與關卡設計，提出遊戲方向調整提案並積極參與討論，促使團隊重新定位製作方向。
                    加入傳送門、引力裝置等設計增加解謎元素，調整遊戲平衡確保挑戰性與趣味性兼具，與團隊共同獲得競賽第三名。
                  </p>
                </li>
                <li className="relative">
                  <span className="absolute -left-[25px] top-1.5 h-1.5 w-1.5 rounded-full bg-foreground" />
              ...
              {projects.map((p, i) => (
                ...
                >
                  <div className={`overflow-hidden ${i === 0 ? "aspect-video md:aspect-[21/9]" : "aspect-video"}`}>
                    <ProjectMedia
                      poster={p.cardPoster ?? p.poster}
                      videoUrl={p.cardPoster ? undefined : p.videoFiles?.find(v => !v.type)?.url}
                      title={p.title}
                      autoPlay
